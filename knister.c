...
void find_best_rest(int** board, int pos, int* verfuegbar, int* verfuegbar_diagonal, int* best_yet, int** best_board)
{
    if(pos == N * N) {
        int new_points = get_board_value(board);
        if(new_points > *best_yet) {
        ...
        for(int k = 0; k < M; k++) {
            if(verfuegbar_diagonal[k] > 0) {
                ...
                find_best_rest(board, pos + 1, verfuegbar, verfuegbar_diagonal, best_yet, best_board);
                verfuegbar_diagonal[k]++;                
            }
        }
    }
...
int main(void)
{
    ...
    int best = 0;
    find_solution2(new_board, verfuegbar, &best, best_board);
